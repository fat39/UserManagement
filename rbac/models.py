...
class User2Group(models.Model):
    ...
    用户角色关系表
    """
    # user = models.ForeignKey(User, verbose_name='用户', related_name='groups',on_delete=models.CASCADE)
    user = models.ForeignKey(User, verbose_name='用户', related_name='groups',on_delete=models.CASCADE)
    group = models.ForeignKey(Group, verbose_name='角色', related_name='users',on_delete=models.CASCADE)
...
class Permission2Action2Group(models.Model):
    """
    权限操作关系表
    """
    ...
    group = models.ForeignKey(Group, verbose_name='角色', related_name='p2as',on_delete=models.CASCADE)
    ...
    def __str__(self):
        return "%s-%s-%s" % (self.permission, self.action, self.group,)