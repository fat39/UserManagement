...
def initial_permission(request, user_id):
    ...
    """初始化权限信息"""
    groups = models.Group.objects.filter(users__user_id=user_id)

    p2a = models.Permission2Action2Group.objects.filter(group__in=groups).values('permission__url',
    ...
    menu_permission_list = list(models.Permission2Action2Group.objects.filter(group__in=groups,
